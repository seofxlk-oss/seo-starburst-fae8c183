...
const extractObjectArray = (source, constName) => {
  const constIndex = source.indexOf(`export const ${constName}`);
  if (constIndex === -1) throw new Error(`Could not find ${constName}`);
...
for (const file of postFiles) {
  ...
  const slug = fileText.match(/slug="([^"]+)"/)?.[1];
  const metaTitle = fileText.match(/metaTitle="([\s\S]*?)"/)?.[1];
  const metaDesc = fileText.match(/metaDesc="([\s\S]*?)"/)?.[1];
  const keywords = fileText.match(/keywords="([\s\S]*?)"/)?.[1];
  if (slug && metaTitle && metaDesc) {
    pageMeta.set(`/blog/${slug}`, {
      ...(pageMeta.get(`/blog/${slug}`) ?? {}),
      title: metaTitle,
